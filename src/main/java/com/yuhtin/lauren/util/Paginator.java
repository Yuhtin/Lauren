...
public class Paginator extends Menu {
    ...
    public void paginate(Message message, int pageNum) {
        if (pageNum < 1)
            ...
            pageNum = pages;
        MessageCreateData msg = renderPage(pageNum);
        initialize(message.editMessage(MessageEditData.fromCreateData(msg)), pageNum);
    }
    ...
    private void paginationWithTextInput(Message message, int pageNum) {
        waiter.waitForEvent(GenericMessageEvent.class, event -> {
            ...
            if (event instanceof MessageReactionAddEvent) {
                ...
                message.editMessage(MessageEditData.fromCreateData(renderPage(targetPage))).queue(m -> pagination(m, targetPage));
    ...
    private void handleMessageReactionAddAction(MessageReactionAddEvent event, Message message, int pageNum) {
        ...
        int n = newPageNum;
        message.editMessage(MessageEditData.fromCreateData(renderPage(newPageNum))).queue(m -> pagination(m, n));
    }

    private MessageCreateData renderPage(int pageNum) {
